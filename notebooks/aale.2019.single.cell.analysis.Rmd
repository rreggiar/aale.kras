...
ifn.signature <- c("DDX58", "IFIH1", "IFNL1", "IRF9", "IFI27", "OAS2", "IFIT3", "IFIT1", "OASL", "ISG15", "OAS3", "RSAD2", "MX1", "OAS1", "IFI44", "CH25H", "IFIT2", "ISG20", "RNASEL", "IFI6", "HERC5", "HERC6", "GBP3", "GBP1", "IRF1", "ISG15")
...
aale_theme <- theme_set(theme_foundation(base_size = 7, base_family = 'Helvetica') + 
...
```{r}
#files <- file.path('../data/for.sce.salmon.14.k.31.gencode.mtx.dump/alevin/quants_mat.gz')
files <- 
  file.path('../data/for.sce.alevin.te.gencode.combine.k.31.no.dups.out/KRAS/alevin/quants_mat.gz')
file.exists(files)
...
#ctrl.aale.sc <- CreateSeuratObject(counts = alevin.ctrl.out, min.cells = 3, min.features = 200, project = "CTRL.AALE")
#kras.aale.sc <- merge(kras.aale.sc, ctrl.aale.sc, add.cell.ids=c('KRAS', 'CTRL'))
...
VlnPlot(kras.aale.sc, features = c("nFeature_RNA", "nCount_RNA", "percent.mt", 'percent.rp'), ncol = 3)
...
kras.aale.sc <- subset(kras.aale.sc, 
                       subset = nFeature_RNA > 800  & percent.mt < 5) # & nFeature_RNA < 6000
kras.aale.sc
...
kras.aale.sc <- FindClusters(kras.aale.sc, resolution = 0.8)
...
FeaturePlot(kras.aale.sc, features = cluster.5.features, pt.size = .42) +
theme_set(theme_foundation(base_size = 7, base_family = 'Helvetica') + 
            theme(plot.title = element_blank(),
                panel.background = element_rect(colour = NA),
                plot.background = element_rect(colour = NA),
...
VlnPlot(kras.aale.sc, features = c("ISG20", "IFI27", "IFIT3", 'IRF9', 'IRF3'), ncol = 3)
...
kras.aale.sc.markers <- 
  FindAllMarkers(kras.aale.sc, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
...
gene.set <- 
  rownames(gene.set.kras.aale.sc[['RNA']]@data)[rownames(gene.set.kras.aale.sc[['RNA']]@data) %in% cluster.5.features]

# Get mean expression of genes of interest per cell
mean.exp <- 
  colMeans(x = gene.set.kras.aale.sc[['RNA']]@data[rownames(gene.set.kras.aale.sc[['RNA']]@scale.data) %in%
                                                     cluster.5.features,], na.rm = TRUE)

# Add mean expression values in 'object@meta.data$gene.set.score'
gene.set.kras.aale.sc@meta.data$cluster.5.features <- mean.exp


kras.sig.vln <- VlnPlot(gene.set.kras.aale.sc, 
        features = c('kras.signal.up'), 
...
hifna.sig.vln <- VlnPlot(gene.set.kras.aale.sc, 
        features = c('msig.df.h.ifna'), 
        nrow = 1,pt.size = 0, size = 0.5, 
        cols = c(viridis(12, option = 'plasma')[1],
                 viridis(12, option = 'inferno')[2],
                 viridis(12, option = 'D')[5],
...
hifng.sig.vln <- VlnPlot(gene.set.kras.aale.sc, 
...
ifn.sig.vln <- VlnPlot(gene.set.kras.aale.sc, 
        features = c('ifn.signature'), 
        nrow = 1,pt.size = 0, size = 0.5, 
        cols = c(viridis(12, option = 'plasma')[1],
                 viridis(12, option = 'inferno')[2],
                 viridis(12, option = 'D')[5],
...
liu.sig.vln <- VlnPlot(gene.set.kras.aale.sc, 
        features = c('liu.ifn.signature'), 
        nrow = 1,pt.size = 0, size = 0.5, 
        cols = c(viridis(12, option = 'plasma')[1],
                 viridis(12, option = 'inferno')[2],
                 viridis(12, option = 'D')[5],
...
# scale_y_continuous(limits = c(-0.1, max(gene.set.kras.aale.sc@meta.data$liu.ifn.signature) ),
  #                    breaks = max(gene.set.kras.aale.sc@meta.data$liu.ifn.signature)) + xlab('cluster') +
  #theme(legend.position = 'NA')



patchwork::wrap_plots(list(kras.sig.vln, hifna.sig.vln, ifn.sig.vln, liu.sig.vln), ncol = 2, guides = 'collect')

...
```
### H: exo alu expression
```{r}
exo.alu.heatmap <- 
  aale.exo.sleuth.out %>%
  filter(te %in% 
           filter(aale.sleuth.out, log2fc > 0)$te) %>% 
  filter(grepl('Alu', te)) %>%
  # select(te, ctrl, kras) %>% 
  # gather(condition, count, -te) %>% 
  # ggplot(aes(te, log(count + 1), color = condition)) + 
  # geom_sina()
  
  group_by(te) %>% 
  summarize(ctrl = mean(ctrl), kras = mean(kras)) %>% 
  gather(condition, count, -te) %>% 
  ...
  pheatmap::pheatmap(color = viridis(512), 
                     ...
                     width = 2.5, height = 1.625, cluster_cols = F)
exo.alu.heatmap

# save_pheatmap_pdf(exo.alu.heatmap, '../figures/fig.3/exo.alu.heat.pdf', 
#                   height = 2.5, width = 1.625)
```

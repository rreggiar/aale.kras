...
te.families.clades <- 
  read_csv('../data/other/reference/te.family.clade.csv',
...
gen.32.lnc <- read_table('../data/other/wetlab/gen.32.pure.lnc.names', col_names = 'gene')
...
gencode.te.locus.aale.norm.counts <- 
  read.csv('../data/bulk.rna.seq/aale/output/gencode.tx.aale.kras.v.ctrl.de-seq.counts.csv') %>% 
  rename(X = 'tx') %>% 
...
ha1em.trono.de.krab.znfs <- read_csv('../data/other/wetlab/all.human.znf.trono.db.csv',
                            col_names = c('gene', 'biotype', 'ensg', 'chr',
                                          'start', 'end', 'strand', 'anno')) %>%
  mutate(anno = gsub('s','', anno),
         anno = gsub('[(]', '', anno),
...
aale.exo.locus.count <- 
  read_csv('../data/bulk.rna.seq/aale/output/tx.exo.gencode.kras.v.ctrl.de-seq.counts.csv') %>% 
  filter(grepl('Alu', X1)) %>% 
  filter_at(vars(contains('.')), any_vars(. >= 10))

# write_csv(x = aale.exo.locus.count,
#           '../data/bulk.rna.seq/aale/output/filt.tx.exo.gencode.kras.v.ctrl.de-seq.counts.csv')
  
exo.aale.de.seq <- 
  read_csv('../data/bulk.rna.seq/exo/output/exo.gencode.kras.v.ctrl.de-seq.csv') %>% 
  filter(padj < 0.01)
names(exo.aale.de.seq) <- c('gene', 'baseMean', 'log2fc', 'lfcse', 'pval', 'padj')

exo.aale.norm.counts <- 
  read_csv('../data/bulk.rna.seq/aale/output/exo.gencode.normalized.counts.csv')

exo.vs.intra.kras.de.seq <- 
  read_csv('../data/bulk.rna.seq/exo.ctrl.compare/output/exo.gencode.intra.v.exo.de-seq.csv',
           col_names = c('gene', 'baseMean', 'log2fc', 'lfcse', 'pval', 'padj'),
           skip=1) %>% 
  filter(padj < 0.01, abs(log2fc) >= 1) 
...
exo.aale.te.de.seq <- 
  read_csv('../data/bulk.rna.seq/aale/output/exo.te.de.seq.csv',
           col_names = c('gene', 'baseMean', 'log2fc', 'lfcse', 'pval', 'padj'),
           skip=1) %>% 
  filter(padj < 0.01, ! gene %in% aale.de.seq$Gene) 
...
### Sleuth -- TEs
```{r}
te.clades.merge <- read_csv('../data/other/wetlab/te.family.clade.csv',
                            col_names = c('te','family','clade')) %>% 
  unique()

aale.sleuth.out <- read_csv('../data/bulk.rna.seq/aale/output/quant.data.log2fc.csv') %>% 
  filter(qval <= 0.05) %>% 
...
aale.exo.sleuth.out <- read_csv('../data/bulk.rna.seq/exo/output/quant.data.log2fc.csv') %>% 
  #filter(qval <= 0.05) %>% 
  separate(Name, sep = '=', into = c('te', 'position', NA, NA, 'strand', NA)) %>% 
  separate(position, sep = '_', into = c('pos', NA)) %>% 
  separate(pos, sep = ':', into = c('chr', 'startend')) %>% 
...
aale.exo.vs.intra.sleuth.out.actual <- read_csv('../data/bulk.rna.seq/exo.kras.te.compare/output/quant.data.log2fc.csv') %>% 
  #filter(qval <= 0.05) %>% 
  separate(Name, sep = '=', into = c('te', 'position', NA, NA, 'strand', NA)) %>% 
  separate(position, sep = '_', into = c('pos', NA)) %>% 
  separate(pos, sep = ':', into = c('chr', 'startend')) %>% 
...
aale.exo.vs.intra.sleuth.out <- read_csv('../data/bulk.rna.seq/exo.kras.te.compare/output/exo.te.aale.kras.v.ctrl.de-seq.counts.csv') %>% 
...
aale.combined.sleuth.out <- 
  read_csv('../data/bulk.rna.seq/aale/output/combined.quant.data.log2fc.csv') %>% 
  filter(qval <= 0.05,
         ! grepl('^ENST', Name)) %>% 
  separate(Name, sep = '=', into = c('te', 'position', NA, NA, 'strand', NA)) %>% 
  separate(position, sep = '_', into = c('pos', NA)) %>% 
...
ha1em.sleuth.out <- read_csv('../data/bulk.rna.seq/ha1em/output/quant.data.log2fc.csv') %>% 
  filter(qval <= 0.05) %>% 
  separate(Name, sep = '=', into = c('te', 'position', NA, NA, 'strand', NA)) %>% 
  separate(position, sep = '_', into = c('pos', NA)) %>% 
  separate(pos, sep = ':', into = c('chr', 'startend')) %>% 
...
# ggsave(plot = isg.ha1e.de.seq,
#        '../figures/fig.1/ha1em.updated.isg.volcano.2.5x3.5.pdf',
#        height = 2.5, width = 3.5, units = 'in')
...
te.aale.sleuth.plt <- 
  ...
  theme(legend.position = 'null')
te.aale.sleuth.plt
# ggsave(plot = te.aale.sleuth.plt, '../figures/fig.3/te-only.aale.te.volcano.2.5x3.5.pdf', 
#        height = 2.5, width = 3.5, units = 'in')
...
# ggsave(plot = te.ha1em.sleuth.plt, '../figures/fig.3/te-only.ha1em.te.volcano.2.5x3.5.pdf',
#        height = 2.5, width = 3.5, units = 'in')
```
...
#ggsave('figures/fig.3/subset.sc.editing.3x3.25.pdf', height = 3, width = 3.25, units = 'in')
```
...
#        height = 2.5, width = 3.5, units = 'in')
ggsave(plot = znf.ha1e.de.seq, '../figures/fig.3/labelled.ha1em.znf.volcano.2.5x3.5.no.legend.pdf',
       height = 2.5, width = 3.5, units = 'in')
```
### B: 
#### ATAC histogram:

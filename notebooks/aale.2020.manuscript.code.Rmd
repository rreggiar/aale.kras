...
gene.set.hist <- function(file.name){
  ...
  input.data %>% 
    ...
    geom_line(alpha = 1, size = rel(0.75)) +
    ...
    scale_y_continuous(limits = c(-5,5), breaks = seq(-12,12, 4)) +
...
collapse.peaks <- read_tsv(paste0(atac.dir, 'peak.files/aale.collapse.merge.sort.peak.bed'), 
  ...
  merge(kras.anno.peaks, by = c('chr', 'start', 'end', 'peak'))
...
aale.anno.collapsed.peaks %>% 
  # filter(gene %in% filter(trono.krab.znfs,
  #                         gene %in% filter(aale.de.seq, log2fc < 1)$gene)$gene) %>%
  filter(gene %in% filter(trono.de.krab.znfs, log2fc < -2)$gene) %>%
  #filter(gene %in% kras.signal.up) %>%
  select(peak, chr, start, end, strand, score, gene, condition, anno) %>% 
  write_tsv(paste0(atac.dir, 'peak.files/total.znf.2.dn.peaks.file'))


total.znf.dn.1.annotated.peaks
...
kras.anno.collapsed.peaks %>% 
  ...
  write_tsv(paste0(atac.dir, 'peak.files/unique.znf.peaks.file'))
...
```  
### peak file histograms:
```{r}


line1.hist <- gene.set.hist('line1.peak.hist') + theme(axis.text.x = element_blank(),
                                                       axis.ticks.x = element_blank(),
                                                       axis.title.x = element_blank(),
                                                       axis.title.y = element_blank())
mer.hist <- gene.set.hist('mer.peak.hist')
patchwork::wrap_plots(list(line1.hist, mer.hist), ncol = 1)
# ggsave('figures/fig.3/stacked.te.peak.density.2.5x3.5.pdf', height = 2.5, width = 3.5, units = 'in')


test.znf.hist <- read_tsv(paste0(atac.dir, 'peak.files/test.znf.hist.txt'),
                          col_names = c('pos', 'ctrl', 'ctrl_pos', 
                                        'ctrl_neg', 'kras', 'kras_pos', 'kras_neg'),
                          skip = 1)
...
# ggsave('figures/fig.4/znf.unique.peak.density.1.5x3.25.pdf', height = 1.5, width = 3.25, units = 'in')

```
### Homer merged/unique peaks mergePeaks:
```{r}

aale.merged.peaks <- read_tsv(paste0(atac.dir, 'peak.files/aale.merged.homer.peaks'),
                              col_names = c('name', 'chr', 'start', 'end', 
                                            'strand', 'stat', 'file', 
                                            'total_subpeaks', 'ctrl_peak', 'kras_peak'),
                              skip = 1) %>% 
  separate_rows(c(name, chr, start, end, strand, stat, file, ctrl_peak, kras_peak), sep = ',') #%>% 
  merge(kras.anno.peaks, by = c('chr', 'start', 'end'))
  
homer.diff.aale.merged.peaks <- read_tsv(paste0(atac.dir, 'peak.files/merged.peaks.diff.tsv'),
                                         col_names = c('name', 'chr', 'start', 'end', 'strand', 'score',
                                                       'focus', 'tot.tags', 'bg.tags', 'fc', 'pval'),
                                         skip = 1) %>% 
  merge(aale.merged.annotated.peaks, by = 'name') %>% 
  merge(aale.de.seq, by = 'gene')

ggplot(homer.diff.aale.merged.peaks, aes(fc, diff)) + 
  geom_point()


aale.merged.annotated.peaks <- read_tsv(paste0(atac.dir, 'peak.files/annotated.aale.merged.homer.peaks'),
                                        col_names = c('name', 'chr', 'start',
                                                      'end', 'strand', 'score',
                                                      'focus', 'annotation',
                                                      'details', 'tss.dist', 'prom.id',
                                                      'entrez.id', 'unigene', 'refseq',
                                                      'ensembl', 'gene', 'alias', 'description',
                                                      'biotype', 'ctrl', 'kras'),
                                        skip = 1) #%>% 
  mutate(diff = log2((kras + 0.01 ) / (ctrl + 0.01))) #%>% 
  filter(gene %in% aale.de.seq$gene, 
         gene %in% kras.isg.signature)
...
kras.unique.peaks <- read_tsv(paste0(atac.dir, 'peak.files/kras.unique.peaks'),
  ...
  write_tsv(paste0(atac.dir, 'peak.files/kras.isg.kras.unique.peaks'))
...
ctrl.unique.peaks %>% 
  ...
  select(name, chr, start, end, strand=strand.y, stat, file, total_subpeaks, peak) %>% 
  filter(chr == 'chr19') %>% 
  #write_tsv(paste0(atac.dir, 'peak.files/de.znf.ctrl.unique.peaks'))
  write_tsv(paste0(atac.dir, 'peak.files/chr19.down.znf.ctrl.unique.peaks'))

combined.unique.peaks <- rbind(kras.unique.peaks, ctrl.unique.peaks)

combined.unique.peaks %>% 
  filter(gene %in% filter(trono.krab.znfs, gene %in% aale.de.seq$gene)$gene) %>% 
  select(chr, start, end, name, strand = strand.x, score, gene, condition, anno) #%>% 
  #write_tsv(paste0(atac.dir, 'peak.files/unique.znf.peaks.file'))

ggplot(combined.unique.peaks %>% filter(gene %in% rig.i.mda5.induction,
                                        abs(tss.dist) <= 1500),
       aes(condition, fill = anno, label = gene)) + 
  geom_bar() + 
  scale_fill_viridis_d()

ggplot(combined.unique.peaks %>% 
         filter(gene %in% kras.isg.signature, abs(tss.dist) <= 1000) %>% 
         gather(),
       aes(anno, y = (..count..), fill = condition)) + 
  geom_violin()
  # geom_bar() + 
  # scale_fill_viridis_d() + 
  # geom_hline(yintercept = seq(0,10, 2), color = 'white', size = rel(1))


combined.unique.peaks %>% 
  filter(gene %in% kras.isg.signature, abs(tss.dist) <= 1000) %>% 
  group_by(condition, tss.dist, anno)
```
### Merged peaks from annoPeaks
```{r}
kras.isg.motifs <- read_tsv(paste0(atac.dir, 'peak.files/kras.isg.motifs.txt')) %>% 
  janitor::clean_names() %>% 
  select_if(function(x) any(!is.na(x))) %>% 
  rename_at(.vars = vars(contains('_')),
            .funs = funs(vapply(str_split(., pattern = '_', n = 2), '[', '', 1))) %>% 
  janitor::clean_names() %>%
  gather(motif, detail, -peak:-kras) %>% 
  separate_rows(peak:detail, sep = '[)],') %>% 
  filter(!is.na(detail)) %>% 
  ...
  mutate(motif = toupper(motif),
         ...
         motif = ifelse(motif == 'AP', 'FOS', motif),
...
# ggsave(plot = kras.isg.motifs.volcano, 'figures/fig.2/updated.kras.isg.volcano.2.5x3.5.pdf', 
       # height = 2.5, width = 3.5, units = 'in')
...
```
### B: TE gene set single cell volcanos
```{r}
...
```
### E: RNA editing in single cells
```{r}
rna.edit.path <- '../data/bulk.rna.seq/aale/output/rna.editing.out/'
rna.edit.path <- '../data/single.cell.rna.seq/output/rna.editing.out/'
rna.edit.path <- '../data/single.cell.rna.seq/output/subset.rna.editing.out/'

plot.editing.matrix <- function(editing.path){
  return(read_csv(paste0(rna.edit.path,paste0(editing.path,'/summary_dir/EditingIndex.csv'))) %>% 
  filter(StrandDecidingMethod == 'RefSeqThenMMSites') %>% 
  select(sample=Sample, signal=A2GEditingIndex, noise=C2TEditingIndex) %>% 
  separate(sample, c(NA, 'cluster', NA), '[.]') %>% 
  ggplot(aes(cluster, y = signal, 
             color = cluster)) + 
  geom_point(size = 2.42) +
  geom_point(aes(cluster, noise), shape = 'diamond') +
...
alu.edit.out <- plot.editing.matrix('sleuth.alu.bed') + xlab('alu')
...
subset.alu.editing.out <- read_csv(paste0(rna.edit.path, 
  ...
  geom_signif(comparisons =  list(c('4','0'),
                                  ...
                                  c('4', '3')),
              color = 'black',
              test = 'wilcox.test', test.args = list(alternative='greater', paired=FALSE),
              step_increase = 0.1, 
              size = rel(0.25), inherit.aes = T, textsize = rel(2)) +
  geom_point(aes(cluster, noise), shape = 'diamond', size = rel(1), alpha = 0.5) + 
  xlab('') + ylab('A-to-G Editing Index') + 
  ...
  theme(plot.margin=margin(-25,-0.5,-2,2,unit = "mm"),
        ...
        axis.title.y = element_text(hjust = -1e-90))

ggsave('../figures/fig.3/subset.sc.editing.2.5x3.25.pdf', height = 2.5, width = 3.25, units = 'in')


line1.edit.out <- plot.editing.matrix('sleuth.line1.bed') + xlab('line1')

mer.edit.out <- 
  read_csv(paste0(rna.edit.path,'sleuth.mer.bed/summary_dir/EditingIndex.csv')) %>% 
  filter(StrandDecidingMethod == 'RefSeqThenMMSites') %>% 
  select(sample=Sample, signal=A2GEditingIndex, noise=C2TEditingIndex) %>% 
  separate(sample, c('condition', 'rep', NA), '[.]') %>% 
...
alu.edit.out <- 
  read_csv(paste0(rna.edit.path,'sleuth.alu.bed/summary_dir/EditingIndex.csv')) %>% 
  filter(StrandDecidingMethod == 'RefSeqThenMMSites') %>% 
  select(sample=Sample, signal=A2GEditingIndex, noise=C2TEditingIndex) %>% 
  separate(sample, c(NA, 'cluster', NA), '[.]') %>% 
  ggplot(aes(cluster, y = signal, 
  ...
  geom_boxplot(size = rel(0.5)) +
  geom_point(size = rel(0.5)) +
  geom_point(inherit.aes = T, aes(cluster, noise), shape = 'diamond') +
  scale_color_manual(values = c(viridis(12, option = 'plasma')[1],
                 viridis(12, option = 'inferno')[2],
                 viridis(12, option = 'D')[5],
                 viridis(12, option = 'plasma')[7],
...
ltr.edit.out <- 
  read_csv(paste0(rna.edit.path,'sleuth.ltr.bed/summary_dir/EditingIndex.csv')) %>% 
  filter(StrandDecidingMethod == 'RefSeqThenMMSites') %>% 
  select(sample=Sample, signal=A2GEditingIndex, noise=C2TEditingIndex) %>% 
  separate(sample, c('condition', 'rep', NA), '[.]') %>% 
  ggplot(aes(condition, signal)) + 
  ...
  facet_wrap(~rep)
line1.edit.out <- 
  read_csv(paste0(rna.edit.path,'sleuth.line1.bed/summary_dir/EditingIndex.csv')) %>% 
  filter(StrandDecidingMethod == 'RefSeqThenMMSites') %>% 
  select(sample=Sample, signal=A2GEditingIndex, noise=C2TEditingIndex) %>% 
  separate(sample, c('condition', 'rep', NA), '[.]') %>% 
  ggplot(aes(condition, signal)) + 
  ...
  facet_wrap(~rep)
total.edit.out <- 
  read_csv(paste0(rna.edit.path,'hg38.out/summary_dir/EditingIndex.csv')) %>% 
  filter(StrandDecidingMethod == 'RefSeqThenMMSites') %>% 
  select(sample=Sample, signal=A2GEditingIndex, noise=C2TEditingIndex) %>% 
  separate(sample, c('condition', 'cluster', NA), '[.]') %>% 
  ggplot(aes(cluster, signal)) + 
  geom_boxplot() + 
  geom_point()+ 
  xlab('line1')
mer.edit.out + alu.edit.out + ltr.edit.out + line1.edit.out
```
...
znf.aale.de.seq
# ggsave(plot = znf.aale.de.seq, '../figures/fig.3/labelled.aale.znf.volcano.2.5x3.5.no.legend.pdf', 
#        height = 2.5, width = 3.5, units = 'in')
```
### B: 
#### ATAC histogram:
